jQuery.fn.toggleSwitch = function (params) {
    ...
    function generateToggle(selectObj) {
        ...
        var $slider = $("<div />").slider({
            min: 0,
            max: 100,
            //animate: "fast", //this was breaking IE
            animate: null,
            change: null,
            stop: function (e, ui) {
        ...
        function toggleValue(slideContain, index) {
          if (isChange(slideContain, index)) {
            options.change.call();
          }
        ...
        function isChange(slideContain, index) {
          var oldIndex = $(slideContain).parent().find("option:selected").index();
          return index != oldIndex && oldIndex != -1;
        }
